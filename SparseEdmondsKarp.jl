module SparseEdmondsKarp
  using LightGraphs 
  """ Code adapted from https://github.com/JuliaGraphs/LightGraphsFlows.jl """
  ...
  """
  function edmonds_karp_sparse_impl(
          residual_graph,               # the input graph
      ...
      flow = 0
      flow_matrix = copy(capacity_matrix)
      fill!(flow_matrix, 0)           # initialize flow matrix. This preserves indices and should be quite efficient.
      P = zeros(Int, n)
      S = zeros(Int, n)
      while true
          fill!(P, -1)
  ...
  function fetch_path!(
      ...
      while true
          if length(Q_f) <= length(Q_r)
              u = pop!(Q_f)
              for v in outneighbors(residual_graph, u)
              ...
              v = pop!(Q_r)
              for u in inneighbors(residual_graph, v)
